...
def print_backtest_report(result: BacktestResult) -> None:
    ...
    print(f"Avg units:         {result.avg_units:,.2f}")
    print(f"Median units:      {result.median_units:,.2f}")
    print(f"Unit range:        {result.min_units:,.2f} -> {result.max_units:,.2f}")
    print("-" * 56)
    print(f"Gross P&L:         ${result.total_gross_pnl:,.2f}")
    print(f"Spread cost:       ${result.total_spread_cost:,.2f}")
    print(f"Entry slip cost:   ${result.total_entry_slippage_cost:,.2f}")
    print(f"Exit slip cost:    ${result.total_exit_slippage_cost:,.2f}")
    print(f"Net P&L:           ${result.total_net_pnl:,.2f}")
    print("-" * 56)
    print(f"Net return:        {result.total_return_pct * 100:,.2f}%")
